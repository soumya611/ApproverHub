...
import { BoxIcon, AnalyticsIcon, JobTrackerIcon, CuBadgeIcon } from "../../icons";
...
const iconMap: Record<string, React.ReactNode> = {
  document: (
    <BoxIcon className="w-5 h-5 group-hover:text-[#007B8C] transition-colors duration-300" />
  ),
  chart: (
    <AnalyticsIcon className="w-5 h-5 group-hover:text-[#007B8C] transition-colors duration-300" />
  ),
  clipboard: (
    <JobTrackerIcon className="w-5 h-5 group-hover:text-[#007B8C] transition-colors duration-300" />
  ),
};
...
export default function DashboardCard({ card }: DashboardCardProps) {
  const content = (
    <div className="group bg-white rounded-xl shadow-[2px_4px_10px_0px_#0000000F] p-6 h-full min-h-[240px] flex flex-col relative overflow-hidden cursor-pointer transition-all duration-300 ease-in-out hover:bg-[#007B8C]">
      ...
      {/* Badge */}
      {card.badge && (
       <div className="absolute bottom-6 left-6 z-10 flex items-center gap-1.5 rounded-full bg-[#DC354514] group-hover:bg-white px-3 py-1 transition-colors duration-300">
          <CuBadgeIcon className="text-[#DC3545]" />
          <span className="text-xs font-semibold text-[#DC3545]">{card.badge}</span>
        </div>
      )}
